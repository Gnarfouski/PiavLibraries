...
internal static class Matrix
{
    ...
    internal static int Rows(this double[,] self)
    {
    ...
    internal static int Cols(this double[,] self)
    {
    ...
    internal static double[,] Transpose(this double[,] self)
    {
        var trans = new double[self.Cols(), self.Rows()];

        for (int row = 0; row < self.Rows(); row++)
        {
            for (int col = 0; col < self.Cols(); col++) trans[col, row] = self[row, col];
        }
    ...
    internal static double[,] Product(this double[,] self, double[,] other)
    {
        if (self.Cols() != other.Rows()) throw new ArgumentException("Invalid matrix size");

        var prod = new double[self.Rows(), other.Cols()];
    ...
    {
        if (self.Cols() != self.Rows()) throw new ArgumentException("Argument matrix is not square!");
        Array.Clear(self, 0, self.Length);

        for (int i = 0; i < self.Rows(); i++) self[i, i] = 1;
    }
    ...
    {
        if (col < 0 || col >= self.Cols()) throw new ArgumentException("Specified column is out of range");

        for (int row = 0; row < self.Rows(); row++) yield return self[row, col];
    }
    ...
    {
        if (row < 0 || row >= self.Rows()) throw new ArgumentException("Specified row is out of range");

        for (int col = 0; col < self.Cols(); col++) yield return self[row, col];
    }
